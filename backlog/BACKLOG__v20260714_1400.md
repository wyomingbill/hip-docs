...
Updated: 2026-07-14 14:00 MT (reconciliation: D-1 commits 1-6 done, secure site live, HARNESS1 green)
Canonical ref: docs/backlog/LATEST_BACKLOG.md -> this file

---

## True State Summary (2026-07-14 14:00 MT)
...
2. D-1 epistemic record writer: commits 1-6 DONE (GAP-1..5 closed, dashboard wired). Commit 7 (shadow retirement) remains OPEN.
3. HARNESS1 all green.
...
7. TD-124 / P2 async write path: measurement instrumented; trial run not yet executed (OPEN).
...
10. Critical path: D-1 commit 7 (shadow retirement) -> DEMO-4 auto-run. Financial validation layer next on business track.
...
Critical path: D-1 commit 7 (shadow retirement) -> DEMO-4 auto-run.
...
|----|-------|----------|--------|--------|--------|-------|
| D-1 | Engine epistemic record writer | P0 | L | **DONE (commits 1-6)** | bf05aae + | GAP-1..5 closed. FLAG-1..8 still in effect. Commit 7 (shadow retirement) OPEN. |
...
| DEMO-4 | Auto-run mode UI | P2 | M | NOT-STARTED | -- | Wire /api/demo/start to FooterBar; beat-aware dwell; focus pane highlighting. Gate: D-1 commit 7. |
| DEMO-5 | Voice mode on /demo surface | P3 | L | NOT-STARTED | -- | Ride OrchestratorGate path; voice = no epistemic record until D-1 fully ships (FLAG-8). |
...
| D-1 commit 7 | Shadow retirement | **OPEN** |
...
- **FLAG-8** Voice = no epistemic record until D-1 fully ships. Silent panes are correct; wrong panes are misleading.
...
|----|-----|-------|--------|-------|
| D-1 commit 7 | GATE | Shadow retirement | **OPEN** | Final D-1 item. Gates DEMO-4. |
| TD-124 / P2 | ENG | Async write path: durable outbox + LiteLLM + DeepInfra fallback | **OPEN** | Measurement instrumented; trial run not yet executed. |
| TD-123 (prompt hardening) | ENG | Detector prompt: "subject must be a PERSON, never the allergen/item" + negative example | **OPEN (P2)** | normalization shipped 47dc59d; prompt hardening pending. |
