...

Part XII: Five-Layer Platform Architecture
The public document describes HIP as a platform with a trust boundary and a context graph. This section discloses the layer structure underneath that description, because the layer boundaries are what make the moat in Part II legally and architecturally defensible, not just narratively defensible.
Layer
Function
Who holds authority
1. Encrypted context
The household's context graph, encrypted at rest and in use, the compounding asset described in Part II.
Household, via the key hierarchy in Part I (modem/router secure element plus member phone secure elements).
2. Recovery authority
The mechanism by which a household regains access to its own context if a key is lost (device replacement, member turnover, household dissolution).
Household-designated recovery path, operator-facilitated but not operator-readable. See 12.1.
3. Consent and rights management
Tracks what each household member has consented to, what the household has opted into (e.g., Data-Sharing tier), and enforces revocation.
Household, per-member, per-consent. Auditable, revocable.
4. Derived intelligence
The routing decisions, tier classifications, and model outputs generated by querying the context graph.
Platform, operating under the constraints layers 1 through 3 impose.
5. Model training
Any process that uses interaction data (never raw context) to improve baseline or specialist models, per the Data-Sharing tier in Part XI.
Platform, strictly isolated from layer 2. See 12.2.

12.1 Recovery authority
A household that loses every enrolled device loses access to its own encrypted context unless a recovery path exists, and the recovery path is the single most sensitive piece of the key architecture, because it is the one place where an operator-facilitated process could, if built wrong, become a backdoor. The recovery mechanism is household-designated: a household selects a recovery method (a secondary member's device, an operator-held encrypted recovery share requiring multi-factor household verification, or a household-generated recovery phrase) at enrollment. The operator can facilitate the recovery workflow. The operator cannot complete it unilaterally. This is the same custody principle from Part II, extended to the failure case rather than only the steady-state case.
12.2 Recovery authority never couples to training authority
This is the single most important isolation boundary in the platform, and it is stated here as an explicit, non-negotiable design constraint rather than an implementation detail. The recovery mechanism in layer 2 and the model-training mechanism in layer 5 must never share code, credentials, or data paths. A recovery event, no matter how it is triggered, cannot expose context to any process that touches training data, and a training pipeline can never be the mechanism, or a component of the mechanism, by which recovery occurs.
The reason this constraint is stated explicitly rather than left as an inference from the architecture diagram is that it is the one place where two legitimate operator interests (helping a locked-out household, improving the platform) could plausibly be merged for engineering convenience, and merging them is precisely what would collapse the trust boundary the entire moat in Part II depends on. Any future engineering proposal that would blur this line is out of scope by design, not by policy that could later be revisited under cost pressure.

Part XIII: Architecture of Trust, NDA Detail
The public document states that the household holds the encryption key and the operator cannot read stored context. This section adds the operational detail an engineering or compliance reviewer will ask for.
13.1 Confidential computing enclave target
The enclave tier described in Part I targets RTX PRO 6000 Blackwell hardware running models via NVIDIA NIM, with confidential computing enabled at the hardware level so that data is encrypted in memory and during processing, not only at rest and in transit. The throughput penalty for running in confidential-computing mode on this hardware class ranges from approximately 5 to 27 percent depending on workload, and that penalty is priced into the enclave tier's per-query variable cost in the Financial Annex rather than absorbed silently.
13.2 Key custody hierarchy
The household's root key is never held by the operator in plaintext form at any point. It is derived and anchored across two device classes: the existing modem or router secure element (fixed, always-on anchor) and each enrolled member's phone secure element (mobile anchor, enables identity outside the home). Both device classes are enrolled into one household key hierarchy at setup. Loss of any single device does not compromise the household's context; loss of all enrolled devices triggers the recovery path in 12.1, not a reset of the household's accumulated context.
13.3 What this enables for regulated partners
The trust architecture is what Part II identifies as the second half of the moat: it is what allows a bank, hospital, or insurer to connect to a household assistant. Concretely, a regulated partner integration operates against layer 3 (consent and rights management), never against raw context in layer 1, and every access is scoped to the specific consent the household granted, auditable, and revocable. This is the mechanism, not the promise, behind the regulated-partner revenue line in the Financial Annex.

...
Part XII, Five-Layer Architecture
Part I, HIP (privacy boundary, inference cascade); Part II, The Moat (trust boundary)
Part XIII, Architecture of Trust Detail
Part I, HIP (confidential computing, key custody); Part II, The Moat (trust boundary as barrier)
