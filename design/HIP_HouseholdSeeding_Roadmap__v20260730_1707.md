...
Status: DESIGN-DRAFT
Branch: roadmap
Reconciled-Against: `HIP_HouseholdSeeding_Roadmap__v20260730_1645.md` (the
prior version, superseded by this filing) revised against two external
critiques (Fable review D-46, parts 1-3; a separate ChatGPT research
review) per D-47. **Provenance note, flagged rather than silently
smoothed over:** at the time of this revision, no captured artifact for
either critique exists in `docs/reviews/` or `docs/dispatches/`, breaking
this project's own established pattern of filing every external review
verbatim before acting on it (see `docs/reviews/FABLE_CuratorReview__*`
and `docs/reviews/FABLE_ShadowScorerReview__*` for that pattern applied
elsewhere). The D-47 dispatch's edit instructions were specific and
self-contained enough to apply directly without re-deriving the critiques;
if the source reviews exist outside this repo, filing them under
`docs/reviews/` (matching the D-46 label already referenced) would close
that provenance gap. Otherwise unchanged from the prior version: no code
exists for any of the five parts below. Where this roadmap touches
already-ratified HIP architecture — trust ladder (ASSERTED/CONFIRMED/
CORROBORATED), the injection contract, partition custody, the write-rule
table — the existing ratified design controls; this document proposes how
a NEW subsystem (confirmation, seeding, boundaries, safety) sits on top of
that ratified layer, not a replacement for it.
...
This roadmap was designed independently of the literatures below; the
Fable review (D-46, parts 1-3) and a separate ChatGPT research review each
independently confirmed that its design choices land inside established,
named bodies of work rather than reinventing untested territory. Listed
here as the grounding those reviews confirmed, not as a literature review
