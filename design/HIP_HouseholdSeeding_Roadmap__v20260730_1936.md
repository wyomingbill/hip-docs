...
Status: DESIGN-DRAFT
Branch: roadmap
Reconciled-Against: `HIP_HouseholdSeeding_Roadmap__v20260730_1707.md` (the
prior version, superseded by this filing), revised per D-48 to fold in four
research-grounded fixes: justification-based trust storage (Part 1), an
evaluation portfolio replacing a self-certifying test plus a named
evaluator/optimizer-separation house rule and a withdrawal-recovery
rule/absolute depth ceiling (Part 3), a re-sorted expert-review gate
(new SUBJECT TO EXPERT REVIEW section), and an extended research-grounding
appendix. **Provenance note, flagged rather than silently smoothed over,
same situation as the prior revision:** at the time of this filing, no
captured artifact for "the four research passes" this dispatch cites exists
in `docs/reviews/` or `docs/dispatches/`, and a broad search of `/tmp`,
this session's scratchpad, and the repo turned up nothing — the same gap
D-47's revision flagged for the Fable/ChatGPT critiques, now also true of
whatever produced the belief-revision/truth-maintenance, Constitutional AI,
construct-validity, Skalse-impossibility, and conversational-repair
citations below. The D-48 dispatch's edit instructions were specific and
citation-complete enough to apply directly without re-deriving the source
material; if the underlying research passes exist outside this repo, filing
them under `docs/reviews/` would close this gap the same way it would close
D-47's. Otherwise unchanged from the prior version: no code exists for any
...
This roadmap was designed independently of the literatures below; external
review — the Fable review (D-46, parts 1-3), a separate ChatGPT research
review, and the additional research passes behind this revision (D-48) —
has repeatedly confirmed that its design choices land inside established,
named bodies of work rather than reinventing untested territory. Listed
here as the grounding those passes confirmed, not as a literature review
this document itself conducted:
