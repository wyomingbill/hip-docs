...
Version: v20260716_1624 MT
Supersedes: HIP_SIA_PhaseB__risk-memo__v20260716_0800.md
Revised after ANALYSIS__postcondition-gap-review__v20260716_1512.md (Fable).
Bill adopted the review's input-side/output-side split. §9 reordered —
two outcome-state exit gates move ahead of everything else; §5.3 corrected;
D-01's place in the sequence stated explicitly; two costs the original
sequence missed are added.
...
Review: docs/research-technical/ANALYSIS__postcondition-gap-review__v20260716_1512.md
...
CORRECTION 2026-07-16 (ANALYSIS__postcondition-gap-review__v20260716_1512.md,
§2): the framing above is wrong about which invariant is load-bearing. G1 and
G4 exist and mis-derive — re-keying them to SIO fields is real work, but it is
a precision fix on invariants that are already running. **The invariant that
actually matters is G0, and G0 exists NOWHERE — not in the harness, not
offline, not in shadow.** `HIP_HarnessPlan__v20260715_1600.md` §3.3 specs it
("G0: reply names a registered member or care recipient while
`resolved_subjects=[]` or nothing admitted about them. Closes G1's blind
spot.") and marks it explicitly **not optional** ("G1 requires
`resolved_subjects` non-empty... A fabrication with `subjects=[]` passes all
four G checks today"). `HIP_DefectRegister__v20260715_1930.md` I-06 cites the
same gap. It was specced, cited twice, and never built. Re-keying G1/G4 to
SIO fields is 5.3's original content and still worth doing; it is not a
substitute for building G0, which this memo previously implied it was.
...
## 9. SEQUENCE — REVISED, OUTPUT-SIDE GATES FIRST

Nothing here waits on a rewrite. Nothing gets done twice.

The original version of this section was entirely input-side: fix the
classifier, then fix the guards that read it. That ordering is wrong. Three
of the four defects motivating this memo (the atorvastatin false ack, D-03,
D-18) are output-state failures — the model fabricates or falsely confirms
regardless of what the classifier says, because nothing downstream gates on
what actually happened. No input-side fix, however correct, touches them.
Bill adopted the split. Two outcome-state exit gates move to the front,
ahead of item 1.

**0. Two outcome-state exit gates.**
- *The atorvastatin false ack* (I-10's cited evidence turn, `[sam] "I take
  atorvastatin 20mg every morning."`). Verified live: `voice_orch.py:3031`
  calls `_gate_unconfirmed_update`, which checks for a zero-write outcome
  only when `_SUPERSEDE_PHRASE_RE.search(query)` matches (the function's
  `if not _SUPERSEDE_PHRASE_RE.search(query): return reply, outcome` early
  exit). The atorvastatin utterance matches none of the supersede-phrase
  keywords (switch/change/stop/start/replace/correct/update/used-to), so the
  zero-write check never runs for it — `reply` passes through unchanged, and
  `voice_orch.py:3097` writes `delta=(_write_outcome or {}).get("delta")`
  (empty) into the very record whose `reply` field, two lines later at
  `:3103`, ships the false ack with no branch on it. Fix: extend the F3
  zero-write check past `_SUPERSEDE_PHRASE_RE` to all declaratives, not just
  supersede-phrased ones. One condition — the mechanism already exists for
  the narrower case, this widens its guard.
- *D-03 / D-18* (confirmation false-positive + instruction leak). Close by
  making the confirmation gate never fall through to the model on an
  unmatched confirmation attempt: no reply parsing, no ungated LLM call
  standing in for a gate the closed vocabulary missed.
- Together: three of the four defects driving this memo close on the output
  side, with zero reply-parsing logic added anywhere. | small, now | — |

**0b. Runtime G0 on the text path.**
Cheap: a closed roster of known-safe reply shapes, case-insensitive
comparison — which asserts away the capitalization-drift objection raised
against G0 in the harness discussion. Ships on the text path only; see
§10 for what that leaves exposed. | small, now | — |

**Why D-01 does not move.** Fail-closed on the classifier's default (D-01)
stays exactly where `HIP_DefectRegister__v20260715_1930.md` put it — Phase 3,
after this exit-gate work. Exit gates convert fabrication into **refusal**;
only the classifier fix restores the **answer**. Different jobs. Shipping
0/0b does not remove the need to fix D-01 eventually, and fixing D-01 first
would not have caught the atorvastatin turn or D-03/D-18 — those are
output-state failures a corrected classifier does not touch. Both are
...
| 3 | **Re-key invariants and oracle to SIO fields.** Runs in shadow. Proves the type expresses the policy before anything depends on it. Precision fix on G1/G4 — not a substitute for building G0 (see §5.3 correction). | tonight-sized | — |
...
Gate integrity (§7) is orthogonal to all of the above and should run in
parallel. It is the reason any of the rest can go quietly wrong.

## 10. TWO COSTS THIS ORDERING DOES NOT ERASE

Both from the review. Neither is solved by 0/0b; both must be said out loud
before that work ships, not discovered after.

**The voice path stays exposed.** Items 0 and 0b above land in
`voice_orch.py`'s text-query path (`process_text_query`). The realtime/voice
adapter (`harness/realtime_adapter.py`) runs its own cascade and does not
share this checkpoint. Until the cascade architecture grows a shared
checkpoint — or the same gates are duplicated into the voice path — closing
0/0b closes the atorvastatin/D-03/D-18 defect class on typed queries only.
Voice keeps the same exposure it has today. This is a text-path fix, not a
system-wide one; say so wherever this memo's sequence is cited.

**A zero-write gate makes P2/i019 user-visible.** Widening the F3 check to
all declaratives (item 0) means every detection miss that used to produce a
silent false ack now produces a spoken refusal instead — including P2/i019's
dietary-preference detection false negative (TD-123/TD-124 territory), which
today fails silently and is invisible to anyone. Shipping item 0 without a
detection retry converts a silent lie into a loud, frequent failure. It must
ship together with a retry on the detection step (or land after TD-123's
prompt-hardening fix lands), not alone.
...
## AMENDMENT PROVENANCE (v20260716_1624)

`ANALYSIS__postcondition-gap-review__v20260716_1512.md` reviewed v0800 and
found its §9 sequence entirely input-side, missing that three of the four
defects motivating this memo are output-state failures no classifier fix
touches. Bill adopted the review's split. This version: §9 reordered (items
0/0b ahead of 1-6), §5.3 corrected (G0 does not exist; re-keying G1/G4 is
not a substitute), D-01's place in the sequence stated explicitly, §10 added
(voice-path exposure, P2/i019 visibility cost), and the review's own §5
provenance flag on this memo's prior unfindability is resolved (see
`docs/research-technical/ANALYSIS__postcondition-gap-review__v20260716_1512.md`
§5.1 and `docs/deliverables/MANIFEST.md` Section B). Everything else —
§1-8, the residue-category argument, the decision table, the 85.7%-is-
agreement-not-accuracy framing, gate integrity as an independent root
cause — stands unchanged from v0800.
